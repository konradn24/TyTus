...
module.exports.run = async (bot, message, args, database) => {
    ...
    database.query(`SELECT * FROM config WHERE name = "${args[0]}"`, (err, rows) => {
        if(err) {
            console.log(err);
            message.channel.send(":x: Wystąpił błąd podczas odczytywania informacji z bazy danych! Spróbuj ponownie później.");
        } else {
            ...
            message.channel.send(`:white_check_mark: Opcja *${args[0]}* jest obecnie ustawiona na: *${rows[0].value}*.`);
        }
    });
}
